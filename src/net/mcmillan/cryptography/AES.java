...
public class AES extends EncryptionScheme {

	public byte[] key = randomKey(1239879242);
	...
	private void encryptBlock(byte[] blk) {
		...
		for (int r=0;r<ROUNDS;r++) {
			...
			subBytes(blk, subkey[0]); // TODO: Make this use Galois field
			shiftRows(blk);
//			mixColumns(blk); TODO: Understand this
	...
	public String decrypt(String coded) {
		...
		return new String(cipherBuffer.array(), charset);
	}
	
	private void subBytes(byte[] blk, byte shift) {
	...
	private void shiftRows(byte[] blk) {
		if (blk.length != BLOCK_AREA) throw new IllegalArgumentException("Array length is " + blk.length);
		for (int y=1;y<BLOCK_SIZE;y++) {
			int fi = y*BLOCK_SIZE;
			byte[] row = Arrays.copyOfRange(blk, fi, fi+BLOCK_SIZE);
			for (int x=0;x<BLOCK_SIZE;x++) blk[fi+x] = row[(x+y)%BLOCK_SIZE];
		}
	}
	
	private void mixColumns(byte[] blk) {
		if (blk.length != BLOCK_AREA) throw new IllegalArgumentException("Array length is " + blk.length);
	...
	private void addKey(byte[] arr, byte[] rndkey) {
